...
public class CommentController {
    ...
    public ResponseEntity<ResponeVo> createComment(@RequestParam("blogId") Long id, String commentContent){
        try {
    ...
    @DeleteMapping("/{id}")
    @PreAuthorize("hasAnyAuthority('ROLE_ADMIN', 'ROLE_USER')")
    public ResponseEntity<ResponeVo> delete(@PathVariable("id") Long id, Long blogId){
        User user = commentService.getCommentById(id).getUser();
        boolean isOwner = false;
        if(SecurityUtils.isBlogOwner(user.getUsername())){
            isOwner = true;
        }
        if(!isOwner){
            return ResponseEntity.ok().body(new ResponeVo(false, "没有权限信息"));
        }
