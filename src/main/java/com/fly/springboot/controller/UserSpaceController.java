...
public class UserSpaceController {
    ...
    @PreAuthorize("authentication.name.equals(#username)")
    public ResponseEntity<ResponeVo> saveAvatar(@PathVariable("username") String username, @RequestBody User user){
        String avatarUrl = user.getAvatar();
        User orgUser = userService.getUserById(user.getId());
        orgUser.setAvatar(avatarUrl);
        userService.saveOrUpdateUser(orgUser);
        return ResponseEntity.ok().body(new ResponeVo(true, "处理成功", avatarUrl));
    }
    ...
    public String listBlogsByOrder(@PathVariable("username") String username,
                                   ...
                                   @RequestParam(value = "catelog", required = false) Long catelogId,
    ...
    public String listBlogsByOrder(@PathVariable("username")String username, @PathVariable("id") Long id, Model model){
        ...
        boolean isOwner = SecurityUtils.isBlogOwner(username);
    ...
    @PreAuthorize("authentication.name.equals(#username)")
    public ResponseEntity<ResponeVo> saveBlog(@PathVariable("username") String username, @RequestBody Blog blog){
        try {
            ...
            return ResponseEntity.ok().body(new ResponeVo(false,
                    ConstraintViolationExceptionHandler.getMessage(e)));
        }
        String redirectUrl = "/u/" + username + "/blogs/" + blog.getId();
        return ResponseEntity.ok().body(new ResponeVo(true, "处理成功", redirectUrl));
    }
    ...
    @PreAuthorize("authentication.name.equals(#username)")
    public ResponseEntity<ResponeVo> deleteBlog(@PathVariable("username") String username, @PathVariable("id") Long id){
        try {
